...
`index.html`을 브라우저로 직접 열어 Playwright로 캡처한 실제 화면입니다.

![커버 섹션](screenshots/01-cover.png)
*커버 — "PORTFOLIO" 타이틀과 한 줄 소개, 우측 하단 프로필 사진. 스크롤 시작 지점.*

![프로젝트 카드 섹션](screenshots/02-project-section.png)
*프로젝트 카드 — 수치 지표(4,000+ 자동 라벨링, 87.3% mAP@50), 회로/현장 사진, 결과 문서 스캔본을 함께 배치한 `project-variant` 레이아웃 예시. 아래로는 다음 프로젝트("쓰리봇") 섹션이 이어진다.*
